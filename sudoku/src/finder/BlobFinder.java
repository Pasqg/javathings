...
public class BlobFinder
{
	...
	static class Blob
	{
		...
		public Blob(int xMin, int xMax, int yMin, int yMax, int mass, int xp, int yp)
		{
			this.xMin = xMin;
			this.xMax = xMax;
			this.yMin = yMin;
		...
		{
			return String.format("X: %4d -> %4d, Y: %4d -> %4d, mass: %6d, area: %d, start: %d,%d", xMin, xMax, yMin, yMax, mass, area(), xp,yp);
		}
	...
	public List<Blob> detectBlobs2(byte[] srcData, byte[] dstData, int minBlobMass, int maxBlobMass, byte matchVal, List<Blob> blobList)
	{
		// This is the neighbouring pixel pattern. For position X, A, B, C & D are checked
		// A B C
		// D X
